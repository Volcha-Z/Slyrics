...
func (m *Model) View() string {
	...
	curLine := m.renderLine(m.styleCurrent, m.state.Lines[m.state.Index].Words)

	if m.Config.Style.OnlyCurrent {
		return gloss.PlaceVertical(m.h, gloss.Center, curLine)
	}
	...
	for filledBefore < beforeLen {
		...
		line := m.renderLine(m.styleBefore, m.state.Lines[beforeIndex].Words)
		beforeIndex -= 1
	...
	for filledAfter < afterLen {
		...
		line := m.renderLine(m.styleAfter, m.state.Lines[afterIndex].Words)
		afterIndex += 1
...
func (m *Model) renderLine(style gloss.Style, s string) string {
	minWidth := widestWordGlyphWidth(s)
	if m.Config.Style.Effect3D {
		minWidth++
	}
	if m.Config.Style.BlockFont && minWidth <= m.w {
		text := alignBlockText(renderBlockText(s, m.w, m.Config.Style.Effect3D), m.w, m.hAlignment)
		return style.Render(text)
	}
	if m.Config.Style.Uppercase {
		s = strings.ToUpper(s)
	}
	return style.Width(m.w).Align(m.hAlignment).Render(s)
}
